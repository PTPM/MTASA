...
function KillMessages ( totalammo, killer, killerweapon, bodypart )
	---These are special checks for certain kill types
	local usedVehicle
	if killerweapon == 19 and isElement(killer) then --rockets
		killerweapon = killer and (getElementType ( killer ) == "player" or getElementType ( killer ) == "ped") and getPedWeapon(killer)
	...
	elseif ( killerweapon == 59 and isElement(killer) ) then
		if ( getElementType ( killer ) == "player" or getElementType ( killer ) == "ped" ) then
	...
	if ( killer and isElement(killer) and (getElementType ( killer ) == "player" or getElementType ( killer ) == "ped")) then
		local kr,kg,kb,ka = exports.ptpm:getPlayerColour( source )
...
addEventHandler ( "onPlayerWasted", getRootElement(), KillMessages )
addEventHandler ( "onPedWasted", getRootElement(), KillMessages )

addEvent ( "onPlayerKillMessage" )
function eventTriggered ( source,killer,weapon,bodypart,suicide,usedVehicle )
	local wr,wg,wb,wa = exports.ptpm:getPlayerColour( source )
	--if getPlayerTeam ( source ) then
		--wr,wg,wb = getTeamColor ( getPlayerTeam ( source ) )
	--end
	local kr,kg,kb,ka = false,false,false,false
	if ( killer ) then
		kr,kg,kb,ka = exports.ptpm:getPlayerColour( killer )
		--if getPlayerTeam ( killer ) then
			--kr,kg,kb = getTeamColor ( getPlayerTeam ( killer ) )
		--end
	end
...
function outputKillMessage ( killed, wr,wg,wb,killer,kr,kg,kb,weapon,width,resource )
	...
	--if not isElement(killed) then
		...
		--return false
	--end
	--if not getElementType(killed) == "player" or getElementType(killed) == "ped" then
		--outputDebugString ( "outputKillMessage - Invalid 'wasted' player specified",0,0,0,100)
		--return false
	--end
