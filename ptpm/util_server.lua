...
function antiSpam( thePlayer )
	--if (not playerInfo) or (not playerInfo[thePlayer]) then return end
	
	local muted = isPlayerMuted( thePlayer )
	if muted then return end
	--if playerInfo[thePlayer].muted then return end
	
	local talkCount = getElementData( thePlayer, "ptpm.spam.talkCount" ) or 0
	local lastTalkTime = getElementData( thePlayer, "ptpm.spam.lastTalkTime" )
	local repeatCount = getElementData( thePlayer, "ptpm.spam.repeatCount" )
	local tick = getTickCount()
	
	--local talkCount = playerInfo[thePlayer].count or 0
	
	talkCount = talkCount + 1
	if lastTalkTime and lastTalkTime + spamTime > tick then
	--if playerInfo[thePlayer].lastTalkTime and playerInfo[thePlayer].lastTalkTime + spamTime > getTickCount() then
		if talkCount > spamLimit then
			repeatCount = repeatCount and repeatCount + 1 or 1
			--playerInfo[thePlayer].repeatCount = playerInfo[thePlayer].repeatCount and playerInfo[thePlayer].repeatCount + 1 or 1
			...
			--if playerInfo[thePlayer].muteTimer then
			--	if isTimer(playerInfo[thePlayer].muteTimer) then
			--		killTimer(playerInfo[thePlayer].muteTimer)
			--	end
			--end		

			local muteTime = false
			if repeatCount == 1 then
			--if playerInfo[thePlayer].repeatCount == 1 then
				muteTime = 30
				setPlayerMuted( thePlayer, true )
				--playerInfo[thePlayer].muted = true
				sendGameText( thePlayer, "You have been muted for 30 seconds for flooding.", 6000, sampTextdrawColours.g, nil, 1.2, nil, nil, 3 )
			elseif repeatCount == 2 then
			--elseif playerInfo[thePlayer].repeatCount == 2 then
				muteTime = 60
				setPlayerMuted( thePlayer, true )
				--playerInfo[thePlayer].muted = true
				sendGameText( thePlayer, "You have been muted for 60 seconds for flooding.\nFINAL WARNING", 6000, sampTextdrawColours.r, nil, 1.2, nil, nil, 3 )
			elseif repeatCount == 3 then
			--elseif playerInfo[thePlayer].repeatCount == 3 then
				local playerName = getPlayerName( thePlayer )
				kickPlayer( thePlayer, "Repeated flooding" )
				outputChatBox( playerName .. " was kicked from the server for repeated flooding.", root, unpack( colourGlobal ) )
				return
			end
			...
			if muteTime then
				muteTimer = setTimer(
					...
					end, muteTime*1000, 1, thePlayer )	
				setElementData( thePlayer, "ptpm.muteTimer", muteTimer, false )
				--playerInfo[thePlayer].muteTimer = setTimer(
				--	function(victim)
				--		sendGameText( victim, "You have been unmuted.", 6000, sampTextdrawColours.g, nil, 1.2, nil, nil, 2 )
				--		playerInfo[victim].muted = nil
				--		playerInfo[victim].mutedTimer = nil
				--	end
				--, muteTime*1000, 1, thePlayer)	
			end
			
			setElementData( thePlayer, "ptpm.spam.repeatCount", repeatCount, false )
			setElementData( thePlayer, "ptpm.spam.talkCount", nil, false )
			setElementData( thePlayer, "ptpm.spam.lastTalkTime", nil, false )
			--playerInfo[thePlayer].count = nil
			--playerInfo[thePlayer].lastTalkTime = nil
			return false
		else
			setElementData( thePlayer, "ptpm.spam.talkCount", talkCount, false )
			setElementData( thePlayer, "ptpm.spam.lastTalkTime", tick, false )
			--playerInfo[thePlayer].count = talkCount
			--playerInfo[thePlayer].lastTalkTime = getTickCount()
		end
	else
		setElementData( thePlayer, "ptpm.spam.talkCount", 1, false )
		setElementData( thePlayer, "ptpm.spam.lastTalkTime", tick, false )
		--playerInfo[thePlayer].count = 1
		--playerInfo[thePlayer].lastTalkTime = getTickCount()
	end
	return true
end
...
addEventHandler("onPlayerInteriorWarped", root,
	function()
		for _, p in ipairs(getElementsByType("player")) do
			if p and isElement(p) and isPlayerActive(p) then
				triggerClientEvent(p, "onClientPlayerInteriorWarped", resourceRoot, source)
			end
		end	
	end
)
